...
class TestLoadAndUpdate:
    ...
    def test_load_and_update_workflow(self, test_services):
        ...
        try:
            ...
            documents, metadatas, ids = prepare_documents(
                services, self.resource_name_211
            )
            ...
            try:
                ...
                for service in update_services:
                    doc, metadata, service_id = prepare_document(
                        service, self.resource_name_211
                    )
            ...
            try:
                ...
                documents, metadatas, ids = prepare_documents(
                    services, self.resource_name_connex
                )
                collection.add(documents=documents, metadatas=metadatas, ids=ids)
    ...
    def test_load_data_with_different_resources_same_collection(self, test_services):
        ...
        collection = self.client.create_collection(
            name="test_different_resources_ontario"
        )
        ...
        try:
            ...
            documents_211, metadatas_211, ids_211 = prepare_documents(
                services_211, "211"
            )
            collection.add(
                documents=documents_211, metadatas=metadatas_211, ids=ids_211
            )
        ...
        try:
            ...
            documents_connex, metadatas_connex, ids_connex = prepare_documents(
                services_connex, "connex"
            )
            collection.add(
                documents=documents_connex, metadatas=metadatas_connex, ids=ids_connex
            )
        ...
        try:
            ...
            documents_empower, metadatas_empower, ids_empower = prepare_documents(
                services_empower, "empower"
            )
            collection.add(
                documents=documents_empower,
                metadatas=metadatas_empower,
                ids=ids_empower,
            )
    ...
    def test_search_by_resource_in_collection(self, test_services):
        ...
        collection = self.client.create_collection(
            name="test_search_by_resource_ontario"
        )
        ...
        results_211 = collection.get(where={"resource": "211"}, include=["metadatas"])
        assert len(results_211["ids"]) == 1
        assert results_211["metadatas"][0]["resource"] == "211"

        results_connex = collection.get(
            where={"resource": "connex"}, include=["metadatas"]
        )

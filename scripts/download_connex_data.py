...
def download_dataset(
    ...
    try:
        ...
        if "RecordCount" not in first_page:
            logger.error(
                f"Unexpected API response format. Keys available: {first_page.keys()}"
            )
            raise ValueError(
                f"API response for dataset {dataset} missing RecordCount field"
            )
        ...
        for page in range(total_pages):
            logger.info(
                f"Dataset {dataset} - Fetching page {page + 1} of {total_pages}"
            )
...
def find_connex_services(
    ...
    connex_services = [
        service for service in cx_services if service["id"] in connex_ids
    ]
